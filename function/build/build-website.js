...
fs.readdirSync("website/src").filter(fileName => fileName.endsWith(".scss") || fileName.endsWith(".sass") ).forEach(sassFile => {
    fs.writeFileSync(`website/dist/${sassFile.replace(/\.s[ca]ss$/,'')}.css`,sass.compile(`website/src/${sassFile}`).css)
